...
本文是对BERT预训练语言模型进行了一个重复性的研究。作者对BERT的预训练进行了仔细的评估，包括**超参数和训练集大小的配置**，发现BERT是被**undertrained**了，将之充分地训练以后可以**反超**在它之后发布的所有模型的效果。

方法其实很简单：

1. 用**更长的时间**，**更大的batch size**，**更多的数据**进行训练。RoBERTA数据量多了10倍
2. 去掉BERT中的**NSP**目标(next sentence prediction)
3. 在**更长的句子**上进行训练
4. 根据训练数据**动态地改变mask**的模式
...
而RoBERTa中使用Dynamic Masking，只是在序列送入模型中的时候才去进行**动态的masking**，这样在更大的数据集上或者更多步数的训练上会表现更好。

#### 去掉NSP任务

NSP（Next Sentence Prediction，下一句预测）任务最初在BERT模型中引入，目的是帮助模型理解句子之间的关系。然而，研究表明，NSP对语言模型的性能提升并不像预期的那样显著，甚至在某些情况下可能会导致性能下降。以下是主要原因：

1. **任务设计的局限性**：NSP任务只是简单地判断两个句子是否相邻，这种二分类任务过于简单，无法帮助模型更好地理解上下文。
2. **增加了训练的复杂性**：在BERT的训练中，NSP任务和MLM（Masked Language Modeling，掩码语言模型）任务是同时进行的。这种联合训练可能会导致任务间的竞争，使得模型无法专注于更有用的语言建模任务。具体而言，NSP任务需要模型关注句子级别的语义一致性，而MLM任务则要求模型关注词汇级别的语言特征，这种任务之间的矛盾可能会干扰模型的优化过程。
3. **任务数据的噪声**：NSP任务的数据构造通常是通过在原文中选择相邻的句子作为正例，而从不同文档中随机选择句子作为负例。然而，这种方法生成的负例可能在语义上与正例没有显著差异，尤其是在大规模语料上进行训练时，这可能导致模型难以学到有效的句子关系信息。
4. **没有显著提升性能**：实验表明，去掉NSP任务并不会显著影响模型在下游任务（如问答、自然语言推理等）上的性能。这表明，MLM任务本身已经足够强大，能够有效学习语言的语法和语义信息，而不需要额外的NSP任务。

**字节对编码（Byte-Pair Encoding, BPE）**：RoBERTa使用了一种基于字节对编码（BPE）的分词器，与BERT相同。但与BERT的WordPiece分词不同，RoBERTa在分词时没有使用字符级别的词汇表。相反，它使用了更大的BPE词汇表（大约50,000个词），这使得RoBERTa在处理语言中的词汇多样性时更加灵活。

#### BPE编码的基本原理

BPE编码的核心思想是通过逐步合并文本中出现**频率最高的字符**或子字符串对，来构建一个新的词汇表。这个过程持续进行，直到达到预定的词汇表大小。

#### BPE编码的步骤：

1. **初始化字符表**：将所有输入文本中的字符初始化为单独的token（词汇单元）。此时，词汇表仅包含这些字符。
2. **统计字符对的频率**：计算文本中所有相邻字符对的出现频率。
3. **合并最频繁的字符对**：找到出现频率最高的字符对，并将其合并为一个新的token（子词单元）。
4. **更新文本表示**：将所有出现该字符对的地方用新的合并字符对替换，更新文本表示。
5. **重复步骤 2-4**：不断重复统计、合并和更新的过程，直到词汇表达到预定的大小或者不再有频繁的字符对可以合并。
