...
这里的特征就是词向量，有**静态(static)和非静态(non-static)**方式。
...
这里使用的是max-pooling，从feature map中选出最大的一个值。也可以改成 **(dynamic) k-max pooling ，pooling阶段保留 k 个最大的信息，保留了全局的序列信息**。
...
:star2:**dropout是指在模型训练时随机让网络某些隐含层节点的权重不工作**，不工作的那些节点可以暂时认为不是网络结构的一部分，但是它的权重得保留下来（只是暂时不更新而已），因为下次样本输入时它可能又得工作了，它是**防止模型过拟合**的一种常用的trick。
